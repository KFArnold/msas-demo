...
To calculate the desired causal effect, we must first specify all key variables which might be relevant for this context.

Examples of key variables therefore include:
...
# and create 'tidy' DAG with node status (i.e. exposure/outcome) & variable labels
...
To estimate this causal effect, we use data collected by the Health Survey for England (HSE) in 2019:
...
# Create tidy summary of fitted model
tidy_summary <- tidy(mod, conf.int = TRUE, exponentiate = TRUE) %>%
...
This model estimates that having an LLTI causes the odds of feeling optimistic about the future to decrease by `r (1 - exp_coef)*100` percent `r paste0("(95% CI: ", paste((1 - exp_conf_high)*100, (1 - exp_conf_low)*100, sep = ", " ), ")")`.
